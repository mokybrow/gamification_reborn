...
async def normilize(db: AsyncSession):
    ...
    while end_of_base:
        ...
        if result:
            # if result[0][0]:
            #     for platform_slug in result[0][0]:
            #         platform_id = select(platforms).where(age_ratings.c.platform_slug == platform_slug)
        ...
        #     # print(result[0][10])
            if result[0][0]:
                for genre in result[0]:
                    query = (select(
                                age_ratings
                                ).where(age_ratings.c.name==genre)
                            )
