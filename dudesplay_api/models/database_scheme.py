...
from sqlalchemy import ARRAY, CHAR, UUID, Boolean, Column, Date, Float, ForeignKey, Integer, MetaData, String, Table, VARCHAR, DateTime, FLOAT, CheckConstraint
...
friends = Table(
    'friends',
    metadata,
    Column('follower_id', UUID, ForeignKey('users.user_id', ondelete="CASCADE"), default=uuid.uuid4()),
    Column('user_id', UUID, ForeignKey('users.user_id', ondelete="CASCADE"), default=uuid.uuid4()),
...
ban_list = Table(
    'ban_list',
    metadata,
    Column('user_id', UUID, ForeignKey('users.user_id', ondelete="CASCADE"), default=uuid.uuid4(), primary_key=True),
...
user_games = Table(
    ...
    Column('user_id', UUID, ForeignKey('users.user_id', ondelete="CASCADE"), default=uuid.uuid4()),
    Column('game_id', UUID, ForeignKey('games.game_id', ondelete="CASCADE"), default=uuid.uuid4()),
    Column('activity_id', UUID, ForeignKey('activity_types.activity_id', ondelete="RESTRICT"), default=uuid.uuid4()),
...
walls = Table(
    'walls',
    metadata,
    Column('wall_id', UUID, primary_key=True, default=uuid.uuid4()), #ID профиля или группы, в будущем
    Column('type_id', UUID, ForeignKey('wall_types.type_id', ondelete="RESTRICT")),
    Column('item_id', UUID, nullable=False), #ID Элемента, котороый принадлежит этой странице
)
...
posts = Table(
    ...
    Column('user_id', UUID, ForeignKey('users.user_id', ondelete="CASCADE"), default=uuid.uuid4()),
    Column('wall_id', UUID, ForeignKey('walls.wall_id', ondelete="CASCADE"), default=uuid.uuid4()),
    Column('parent_post_id', UUID, ForeignKey('posts.post_id', ondelete="CASCADE"), default=uuid.uuid4()),
...
post_tags = Table(
    'post_tags',
    metadata,
    Column('post_id', UUID, ForeignKey('posts.post_id', ondelete="CASCADE"), default=uuid.uuid4()),
    Column('tag_id', UUID, ForeignKey('tags.tag_id', ondelete="CASCADE"), default=uuid.uuid4()),
)
...
comments = Table(
    ...
    Column('parent_comment_id', UUID, ForeignKey('comments.comment_id', ondelete="CASCADE"), default=uuid.uuid4()),
    Column('like_count', Integer, nullable=True),
)
